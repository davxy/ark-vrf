...
impl crate::ring::RingSuite for ThisSuite {
    ...
    const ACCUMULATOR_BASE: AffinePoint = {
        const X: BaseField = MontFp!(
            "45073877898970548159290209639997145354159827840789102840657890343862981627277"
        );
        const Y: BaseField = MontFp!(
            "39295673701369313144962461808865505415635063815454350746484753351079386979596"
        );
        AffinePoint::new_unchecked(X, Y)
    };
