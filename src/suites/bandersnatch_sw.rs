...
impl PedersenSuite for ThisSuite {
    const BLINDING_BASE: AffinePoint = {
        const X: BaseField =
            MontFp!("4396245810281999292281250604408272257121889347880230507213334594513680840433");
        const Y: BaseField =
            MontFp!("7864841271910327444965646028953088795529105295371536376793889880750530283707");
...
impl crate::ring::RingSuite for ThisSuite {
    ...
    const ACCUMULATOR_BASE: AffinePoint = {
        const X: BaseField = MontFp!(
            "37204288872082471308108733990654952710168405905466025073818174038668396842259"
        );
        const Y: BaseField = MontFp!(
            "51955035813088797343165479522308310984096932187413446899876574911734533607047"
        );
        AffinePoint::new_unchecked(X, Y)
    };
    ...
    const PADDING: AffinePoint = {
        const X: BaseField =
            MontFp!("1367719337332655367193475775043467205002762327769312386653619372379936863336");
        const Y: BaseField =
            MontFp!("4621036458700603848544645692949366454919324002011261784197649061986895304486");
        AffinePoint::new_unchecked(X, Y)
    };
}
