...
- `Suite::nonce` now takes an additional `ad: &[u8]` parameter for
  domain-separated nonce generation.
- Pedersen prover cross-binds the two secrets into nonce additional data:
  `k` nonce includes `encode(blinding) || ad` and `kb` nonce includes
  `encode(secret) || ad`.
- `nonce_rfc_8032` and `nonce_rfc_6979` utility functions updated accordingly.
